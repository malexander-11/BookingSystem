...
| Handout (not in the menu) | `handout.html` | Standalone post-session revision handout: the golden thread, everything the group produced at each stage, the fake results, takeaways and a glossary. One self-contained file generated from the four configs by `node scripts/build-handout.mjs`; can be emailed or printed | Nothing |
