...
| Handout (not in the menu, URL only) | `handout.html` | Standalone post-session revision handout generated from the four configs (`node scripts/build-handout.mjs`): what the group produced at each stage, the fake results, takeaways, glossary. One file, emailable, prints to PDF | Nothing |
